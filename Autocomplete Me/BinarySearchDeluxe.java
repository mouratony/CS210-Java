...
public class BinarySearchDeluxe {
    ...
    public static <Key> int firstIndexOf(Key[] a, Key key, Comparator<Key> c) {
        // Throw and error message if a, key, or c is null.
        if (a == null || key == null || c == null) {
            throw new NullPointerException("a, key, or c is null");
        }
        // Apply the Binary search code, but modify that when we have a match,
        // set index to mid, and set hi to mid - 1.
        int index = -1;
        int lo = 0;
        int hi = a.length - 1;
        while (lo <= hi) {
    ...
    public static <Key> int lastIndexOf(Key[] a, Key key, Comparator<Key> c) {
        ...
        // Apply the Binary search code, but modify that when we have a match,
        // set index to mid, and set lo to mid + 1.
        int index = -1;
        int lo = 0;
        int hi = a.length - 1;
        while (lo <= hi) {
